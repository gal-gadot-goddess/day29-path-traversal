...
async function automateDay29() {
    console.log(`🚀 DAY 29 AI-POWERED AUTOMATION`);

    try {
        console.log("🤖 Generating a unique AI scenario for Dijkstra...");
        const genScript = path.join(__dirname, 'scripts/generate_new_dijkstra_topic.mjs');
        await runCommand('node', ["\"" + genScript + "\""], { cwd: __dirname });

        // Load the newly generated topic to get title/desc
        ...
        await runCommand('node', ['capture_day29.mjs'], { cwd: __dirname });

        console.log("Generating AI Metadata...");
        const metaScript = path.join(__dirname, 'scripts/generate_ai_metadata.mjs');
        await runCommand('node', ["\"" + metaScript + "\"", `"${topicData.algorithm}"`, `"${topicData.description}"`], { cwd: __dirname });
        ...
        console.log("Day 29 Generation Success!");
